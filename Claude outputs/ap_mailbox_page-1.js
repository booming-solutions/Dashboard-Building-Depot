...
// Statussen die als "afgehandeld" tellen voor de productiviteitsmeting.
...
// Personen die ALTIJD toewijsbaar zijn in de mailbox, ongeacht hun rol (naast alle ap_clerk/admin).
// Voeg hier een e-mailadres toe om iemand selecteerbaar te maken zonder zijn/haar rol te wijzigen.
...
export default function MailboxPage() {
  ...
  // Manager = admin óf op de allowlist. Gebruikt om het "voorstellen"-blok (dat vaste
  // toewijsregels maakt) voor te behouden aan managers. Toewijzen zelf mag iedereen.
  // We lezen dit bij voorkeur uit de layout (ctx.isManager) met lokale fallback.
  const MANAGER_EMAILS_FALLBACK = ['lucavanderbreggen@gmail.com'];
  ...
  const [expDay, setExpDay] = useState(() => new Date().toLocaleDateString('en-CA')); // exportdag in de werklijst
  ...
  async function assign(row, clerkId) {
    ...
    // Iedereen mag toewijzen én herverdelen: een regel kan aan iedereen worden
    // toegewezen, teruggezet in de voorraad, of aan een ander gegeven. Wie wat
    ...
    await patch(row.id, fields, 'toegewezen', { clerk: nameFull(target) || '—' });
  }
  ...
  const werklijst = useMemo(() => {
    ...
    order.forEach((k) => {
      ...
      groups.push({ key: k, clerkId, label, total: items.length, late, buckets, items });
    });
    return groups;
  }, [rows, passNoClerk, dueBucket, todayStr, clerkName]);

  // DASHBOARD — totale werkvoorraad + ouderdom, en productie per persoon per week.
  // "Afgehandeld/productie" telt op wie de regel oploste (resolved_by/resolved_at).
  ...
  // Werklijst-export: zet een lijst mails om naar Excel-rijen (persoon + deadline vooraan).
  async function exportWL(list, filename, title) {
    const sorted = [...list].sort((a, b) => {
      const an = (clerkName(a.assigned_clerk) || 'zzz').toLowerCase(), bn = (clerkName(b.assigned_clerk) || 'zzz').toLowerCase();
    ...
    const out = sorted.map((r) => ({
      Persoon: clerkName(r.assigned_clerk) || '— niet toegewezen —',
      Deadline: r.due_date ? new Date(r.due_date).toLocaleDateString('nl-NL') : '',
      'Te laat': isOverdue(r) ? 'JA' : '',
      ...
      Onderwerp: r.subject || '',
      Status: r.status,
      Ontvangen: fmtDate(r.received_at),
    }));
    await exportToExcel({ filename, reportTitle: title, sheets: [{ name: 'Werklijst', rows: out }] });
  }

  // Exporteer alle openstaande taken met deadline = gekozen dag (respecteert zoek/persoon/entiteit-filter).
  async function exportDay() {
    const list = rows.filter(FOLDERS[0].test).filter(passFilter).filter((r) => r.due_date === expDay);
    const nl = new Date(expDay).toLocaleDateString('nl-NL', { weekday: 'long', day: '2-digit', month: '2-digit', year: 'numeric' });
    await exportWL(list, `AP_Werklijst_${expDay}`, `Werklijst — ${nl}`);
  }

  // Exporteer de volledige openstaande lijst van één persoon (alle deadlines).
  async function exportPerson(group) {
    await exportWL(group.items, `AP_Werklijst_${(group.label || 'persoon').replace(/\s+/g, '_')}`, `Werklijst — ${group.label} (alle deadlines)`);
  }

  // Exporteer de productiviteit per persoon naar Excel (manager).
  async function exportProductivity() {
    const out = productivity.list.map((a) => ({
      Persoon: a.name,
      'Open voorraad': a.open,
      'Te laat': a.late,
      'Afgehandeld 7 dagen': a.done7,
      'Afgehandeld 30 dagen': a.done30,
      'Afgehandeld totaal': a.doneTotal,
      'Gem. doorlooptijd (dagen)': a.cycle != null ? Math.round(a.cycle * 10) / 10 : '',
    }));
    await exportToExcel({
      filename: 'AP_Productiviteit',
      reportTitle: `AP Mailbox — productiviteit per persoon (${new Date().toLocaleDateString('nl-NL')})`,
      sheets: [{ name: 'Productiviteit', rows: out }],
    });
  }
  ...
  const assignSelect = (r, compact) => {
    const avatar = r.assigned_clerk && (
      <span className="inline-flex items-center justify-center w-[22px] h-[22px] rounded-full text-white text-[10px] font-bold shrink-0" style={{ background: clerkColor(r.assigned_clerk) }}
        title={nameFull(r.assigned_clerk)}>
        {initials(nameFull(r.assigned_clerk))}
      </span>
    );
    // Iedereen krijgt de volledige dropdown: toewijzen aan wie dan ook,
    // terugzetten in de voorraad, of aan een ander geven (herverdelen).
    return (
  ...
  return (
    <div className="max-w-[1500px]">
      ...
      {/* voorstellen — leren met melding (alleen managers: dit herverdeelt werk) */}
      ...
      {view === 'productiviteit' ? (
        ...
        <div>
          ...
          {/* Dag-export: kies een dag → exporteer alles wat die dag af moet (per persoon), respecteert de filters hierboven */}
          <div className="flex items-center gap-2 flex-wrap mb-3 bg-[#f7f9fc] border border-gray-200 rounded-lg px-3 py-2">
            <span className="text-[12px] font-semibold text-[#1B3A5C]/70">📅 Dag exporteren</span>
            <input type="date" value={expDay} onChange={(e) => setExpDay(e.target.value)}
              className="text-[13px] px-2 py-1 border border-gray-200 rounded-lg bg-white" />
            <button onClick={exportDay} disabled={busy}
              className="text-[12px] font-semibold px-3 py-1.5 rounded-lg bg-[#1f8a52] text-white hover:bg-[#186e41] disabled:opacity-50">
              ⬇ Exporteer deze dag (Excel)
            </button>
            <span className="text-[12px] text-[#1B3A5C]/45">
              alles met deadline op {new Date(expDay).toLocaleDateString('nl-NL', { weekday: 'long', day: '2-digit', month: '2-digit' })}
              {fClerk ? ` · alleen ${scopeLabel(fClerk)}` : ''}
            </span>
            <span className="ml-auto text-[12px] text-[#1B3A5C]/50">Loop de lijst af, zet per mail een persoon én een streefdatum. Te laat = rood.</span>
          </div>
          ...
          <div className="space-y-4">
            {werklijst.filter(wlGroupMatch).map((g) => (
              <div key={g.key} className="border border-gray-200 rounded-xl bg-white overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-4 py-2.5 bg-[#f7f9fc] border-b border-gray-200">
                  ...
                  <button onClick={() => exportPerson(g)} disabled={busy}
                    title="Exporteer de volledige lijst van deze persoon (alle deadlines) naar Excel"
                    className="ml-auto text-[11.5px] font-semibold px-2.5 py-1 rounded-lg border border-gray-200 bg-white text-[#1B3A5C] hover:bg-gray-50 disabled:opacity-50">⬇ Excel</button>
                </div>
