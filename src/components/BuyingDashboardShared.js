/* ============================================================
   BESTAND: BuyingDashboardShared.js
   KOPIEER NAAR: src/components/BuyingDashboardShared.js
...
export default function BuyingDashboardShared({ bumFilter }) {
  var _s = useState;
  ...
  async function loadData() {
    ...
    while (true) {
      ...
      // Filter on database level for speed
  ...
  var title = bumFilter ? 'Inkoopvoorstel — ' + bumFilter : 'Inkoopvoorstel — Totaaloverzicht';
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-6">
      ...
      <p className="text-[13px] text-[#6b5240]">{'Inkoopvoorstel laden' + (bumFilter ? ' (' + bumFilter + ')' : '') + '...'}</p>
    </div>
  );
  ...
  return (
    <div className="max-w-[1700px] mx-auto" style={{ fontFamily: "'DM Sans', -apple-system, sans-serif", color: '#1a0a04' }}>
      ...
      {/* Filters — no BUM filter needed when bumFilter is set */}
