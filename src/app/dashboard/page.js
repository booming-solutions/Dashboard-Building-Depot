...
export default function OverviewPage() {
  ...
  async function loadData() {
    try {
      ...
      let allSales = [], from = 0;
      while (true) {
        const { data: b } = await supabase.from('sales_monthly').select('*').order('year').order('month').range(from, from + 999);
        if (!b || !b.length) break; allSales = allSales.concat(b); if (b.length < 1000) break; from += 1000;
      }
      ...
      setSalesData({
        monthSales: sum(cyMonth, 'net_sales'), monthGM: sum(cyMonth, 'gross_margin'),
        monthGMpct: sum(cyMonth, 'net_sales') ? sum(cyMonth, 'gross_margin') / sum(cyMonth, 'net_sales') * 100 : 0,
        monthLySales: sum(lyMonth, 'net_sales'), ytdSales: sum(cyYTD, 'net_sales'), ytdGM: sum(cyYTD, 'gross_margin'),
        ytdGMpct: sum(cyYTD, 'net_sales') ? sum(cyYTD, 'gross_margin') / sum(cyYTD, 'net_sales') * 100 : 0,
        ytdLySales: sum(lyYTD, 'net_sales'), currentMonth,
      });
    } catch (e) { console.error(e); }
    setLoading(false);
  }

  const totalBudget = 21776079, totalInvY = 21316338, invPct = totalInvY / totalBudget * 100;
  ...
  return (
    <div className="max-w-[1520px] mx-auto" style={{ fontFamily: "'DM Sans',-apple-system,sans-serif", color: '#1a0a04' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ fontFamily: "'Playfair Display',Georgia,serif", fontSize: '22px', fontWeight: 900 }}>Dashboard Overzicht</h1>
      ...
      {loading ? (
        ...
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">📈</span><h2 className="text-[16px] font-bold">Omzet & Marge</h2><span className="text-[11px] text-[#6b5240] ml-1">— {MN[(salesData?.currentMonth||1)-1]} 2026 · Curaçao</span></div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              ...
              <Link key={i} href="/dashboard/sales" className="block group">
                <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-5 relative overflow-hidden shadow-sm group-hover:border-[#E84E1B] group-hover:shadow-md transition-all">
                  <div className="absolute top-0 left-0 right-0 h-[3px]" style={{backgroundColor:kpi.color}}/><p className="text-[11px] text-[#6b5240] font-bold uppercase tracking-[1px]">{kpi.label}</p>
        ...
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">📦</span><h2 className="text-[16px] font-bold">Voorraad</h2><span className="text-[11px] text-[#6b5240] ml-1">— Curaçao · per 26 Mrt 2026</span></div>
        ...
        <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-5 shadow-sm">
          <h3 className="text-[14px] font-bold mb-3">Snelle Links</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {[{href:'/dashboard/sales',icon:'📈',t:'Omzet en Marge',d:'Volledig sales dashboard'},{href:'/dashboard/inventory/budget',icon:'📦',t:'Voorraad vs Budget',d:'Inventory per department'},{href:'/dashboard/reports',icon:'📋',t:'Rapportages',d:'Overzichten & exports'},{href:'/dashboard/files',icon:'📁',t:'Bestanden',d:'Documenten & uploads'}].map((l,i)=>(
              <Link key={i} href={l.href} className="flex items-center gap-3 p-3 rounded-lg border border-[#e5ddd4] hover:border-[#E84E1B] hover:bg-[#faf5f0] transition-all">
                <span className="text-xl">{l.icon}</span><div><p className="text-[13px] font-semibold">{l.t}</p><p className="text-[11px] text-[#6b5240]">{l.d}</p></div>
              </Link>
            ))}
          </div>
        </div>
