/* ============================================================
   BESTAND: sandbox_ap_match_worklist_page_v5.js
   KOPIEER NAAR: src/app/dashboard/finance/sandbox-ap/match/worklist/page.js
   (overschrijft sandbox v4, hernoemen naar page.js)
   🧪 SANDBOX-MIRROR van productie v5 — regel-voor-regel identiek aan live,
   alleen aangepast:
...
export default function MatchWorklistPage() {
  ...
  async function confirmSelected() {
    ...
    try {
      ...
      const { error: e1 } = await supabase.from('sandbox_ap_match_candidates')
      ...
      for (const [date, invIds] of Object.entries(byDate)) {
        ...
        const { error: e2 } = await supabase.from('sandbox_ap_invoices')
          .update({
            status: 'paid',
            paid_at: date,
