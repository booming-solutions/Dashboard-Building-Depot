...
async function executeImport(supabase, parsedInvoices, diff, currentUser, filename, entity) {
  ...
  if (diff.newVendors.length > 0) {
    ...
    const { error } = await supabase.from('ap_vendors').insert(rows);
    if (error) errors.push(`Vendor insert: ${error.message}`);
  }

  const { data: vendors } = await supabase.from('ap_vendors').select('vendor_id, assigned_bum');
  ...
  if (diff.newInvoices.length > 0) {
    ...
    for (let i = 0; i < newRows.length; i += BATCH) {
      ...
      const { error, count } = await supabase.from('ap_invoices').insert(batch, { count: 'exact' });
      if (error) {
        ...
        for (const row of batch) {
          const { error: rowErr } = await supabase.from('ap_invoices').insert([row]);
          if (rowErr) {
            perRowFail++;
