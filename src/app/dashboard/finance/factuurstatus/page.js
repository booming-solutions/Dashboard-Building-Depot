/* ============================================================
   BESTAND: factuurstatus_page_v8.js
   ...
   v8:
   - Databanner leest nu uit ledger_load_log (de geautomatiseerde
     mail-import): laatste run, aantal regels, +nieuw, afgevoerd,
     bronbestand en status.
   - Valt terug op invoice_ledger.loaded_at als het log leeg is
     (bv. na een handmatige upload) — dan zonder mutatiecijfers.
   - Waarschuwing als de laatste run niet op status 'ok' staat.
   v7:
   - BDMS verwijderd uit de entiteit-knoppen.
   - Waarschuwing over toekomstige factuurdatums verwijderd.
   v6:
   - Databanner bovenaan met stoplicht.
   v5:
...
export default function FactuurStatusPage() {
  ...
  // Databanner: laatste run uit ledger_load_log, met fallback op loaded_at
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setMetaLoading(true);
      try {
        const [logRes, countRes] = await Promise.all([
          supabase.from('ledger_load_log')
            .select('run_at, source_file, rows_after, rows_new, rows_stale, status, message')
        ...
        if (log) {
          setMeta({
            ...
            totalRows:  countRes.count ?? log.rows_after ?? null,
          ...
          // Nog geen geautomatiseerde run: val terug op de ledger zelf
          const fb = await supabase.from('invoice_ledger')
          ...
          setMeta({
            source: 'ledger',
            loadedAt:   fb.data?.[0]?.loaded_at || null,
            sourceFile: fb.data?.[0]?.source_file || null,
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {/* Databanner: laatste dataload + mutaties */}
      <div className="mb-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-xl border border-gray-200 bg-white px-4 py-2.5">
        {metaLoading ? (
          ...
          <>
            ...
            {meta.totalRows !== null && (
              <span className="text-[11px] text-[#1B3A5C]/50">
                {fmtInt(meta.totalRows)} regels in de ledger
              </span>
            )}
            {meta.rowsNew !== null && meta.rowsNew !== undefined && (
              <span className="text-[11px] font-semibold text-emerald-700">
                +{fmtInt(meta.rowsNew)} nieuw
              </span>
            )}
            {meta.rowsStale !== null && meta.rowsStale !== undefined && meta.rowsStale > 0 && (
              <span className="text-[11px] font-semibold text-amber-700" title="Facturen die binnen het exportvenster vallen maar niet meer in de laatste levering stonden">
                {fmtInt(meta.rowsStale)} afgevoerd
              </span>
            )}
            {meta.sourceFile && (
              <span className="text-[11px] text-[#1B3A5C]/40 font-mono truncate max-w-[260px]" title={meta.sourceFile}>
                {meta.sourceFile}
              </span>
            )}
          </>
      ...
      {/* Waarschuwing als de laatste import niet schoon is verlopen */}
      {!metaLoading && meta?.status && meta.status !== 'ok' && (
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-[12px] text-amber-800">
          <span className="font-semibold">Laatste import: {meta.status}.</span>{' '}
          {meta.message || 'Controleer de logs van de import.'}
        </div>
      )}
      {!metaLoading && (!meta?.status || meta.status === 'ok') && <div className="mb-4" />}
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="mt-3 text-[11px] text-[#1B3A5C]/40">
            Bron: dagelijkse Compass-export (AI Open and Paid Items last 12M). "Betaald" betekent volledig afgeletterd; "Open" staat nog uit.
