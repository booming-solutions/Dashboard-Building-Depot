...
export default function SalesDashboard(){
  ...
  async function loadData(){
    ...
    // Get max date for YTD proration
    const{data:maxDateRows}=await supabase.from('sales_data').select('sale_date').order('sale_date',{ascending:false}).limit(1);
    if(maxDateRows&&maxDateRows.length)setRawDates(maxDateRows);
    setData(allSales);
  ...
  const isYTD=months.includes('ytd');
  const isAll=months.includes('all');
  const selectedMonths=useMemo(()=>{
    if(isAll||isYTD)return null;
    return months.map(m=>parseInt(m)).filter(m=>!isNaN(m));
  },[months,isAll,isYTD]);

  // YTD proration: what % of the last month do we have data for?
  const ytdProration=useMemo(()=>{
    if(!rawDates.length)return{month:0,dayFraction:1};
    const lastDate=new Date(rawDates[0].sale_date);
    const lastDay=lastDate.getDate();
    const daysInMonth=new Date(lastDate.getFullYear(),lastDate.getMonth()+1,0).getDate();
    return{month:lastDate.getMonth()+1,dayFraction:lastDay/daysInMonth};
  },[rawDates]);
  ...
  function matchesMonth(m){
    if(isAll)return true;
    if(isYTD)return m<=maxDataMonth;
    if(selectedMonths)return selectedMonths.includes(m);
    return true;
  }
  ...
  const filtered=useMemo(()=>data.filter(r=>{
    ...
    return matchesMonth(r.month);
  }),[data,store,currentYear,months,bum,dept,maxDataMonth]);
  ...
  const priorFiltered=useMemo(()=>data.filter(r=>{
    ...
    return matchesMonth(r.month);
  }),[data,store,priorYear,months,bum,dept,maxDataMonth]);
  ...
  const budgetFiltered=useMemo(()=>budgetData.filter(b=>{
    ...
    if(!matchesMonth(bm))return false;
    return b.budget_type===salesType||b.budget_type===marginType;
  }),[budgetData,store,currentYear,months,dept,budgetMode,maxDataMonth,salesType,marginType]);

  const sum=(arr,key)=>arr.reduce((s,r)=>s+parseFloat(r[key]||0),0);

  // Apply YTD proration to LY and budget for the partial month
  function prorateLY(total){
    if(!isYTD||!ytdProration.month)return total;
    // Prorate the last month of LY data
    const lastMonthLY=priorFiltered.filter(r=>r.month===ytdProration.month);
    const lastMonthSales=lastMonthLY.reduce((s,r)=>s+parseFloat(r.net_sales||0),0);
    const adjustment=lastMonthSales*(1-ytdProration.dayFraction);
    return total-adjustment;
  }
  function prorateLYGM(total){
    if(!isYTD||!ytdProration.month)return total;
    const lastMonthLY=priorFiltered.filter(r=>r.month===ytdProration.month);
    const lastMonthGM=lastMonthLY.reduce((s,r)=>s+parseFloat(r.gross_margin||0),0);
    return total-lastMonthGM*(1-ytdProration.dayFraction);
  }
  function prorateBudget(total,type){
    if(!isYTD||!ytdProration.month)return total;
    const lastMonthBud=budgetFiltered.filter(b=>{const bm=parseInt(b.month.split('-')[1]);return bm===ytdProration.month&&b.budget_type===type});
    const lastMonthAmt=lastMonthBud.reduce((s,b)=>s+parseFloat(b.amount||0),0);
    return total-lastMonthAmt*(1-ytdProration.dayFraction);
  }

  const isBonaire=store==='B';
  const curr=isBonaire?'US$':'XCG';
  const conv=v=>isBonaire?v/XCG_USD:v;
  const fmtC=n=>fmt(conv(n));
  const fmtMC=n=>fmtM(conv(n));
  ...
  const rawLySales=sum(priorFiltered,'net_sales');
  const rawLyGM=sum(priorFiltered,'gross_margin');
  const lySales=prorateLY(rawLySales);
  const lyGM=prorateLYGM(rawLyGM);
  const lyGmPct=lySales?lyGM/lySales*100:0;
  const rawBudgetSales=sum(budgetFiltered.filter(b=>b.budget_type===salesType),'amount');
  const rawBudgetMargin=sum(budgetFiltered.filter(b=>b.budget_type===marginType),'amount');
  const budgetSales=prorateBudget(rawBudgetSales,salesType);
  const budgetMargin=prorateBudget(rawBudgetMargin,marginType);
  ...
  function handleMonthClick(m,e){
    if(m==='all'||m==='ytd'){setMonths([m]);return}
    if(e&&e.ctrlKey){
      setMonths(prev=>{
        const clean=prev.filter(x=>x!=='all'&&x!=='ytd');
        if(clean.includes(m))return clean.filter(x=>x!==m).length?clean.filter(x=>x!==m):['all'];
        return[...clean,m];
      });
    }else{setMonths([m])}
  }
  ...
  const renderCharts=useCallback(()=>{
    ...
    // Apply proration to last month in YTD mode
    if(isYTD&&ytdProration.month){
      const pm=ytdProration.month-1;
      if(lyMonthly[pm]){lyMonthly[pm].s*=ytdProration.dayFraction;lyMonthly[pm].g*=ytdProration.dayFraction}
      if(budMonthly[pm]){budMonthly[pm].s*=ytdProration.dayFraction;budMonthly[pm].g*=ytdProration.dayFraction}
    }

    let allMonths=[];
    if(selectedMonths&&selectedMonths.length){allMonths=selectedMonths.map(m=>m-1).sort((a,b)=>a-b)}
    else{
    ...
    // Calculate Y-axis min for better perspective
    const allSalesValues=[...allMonths.map(m=>conv(curMonthly[m]?.s||0)),...allMonths.map(m=>conv(lyMonthly[m]?.s||0)),...allMonths.map(m=>conv(budMonthly[m]?.s||0))].filter(v=>v>0);
    const minSales=allSalesValues.length?Math.min(...allSalesValues):0;
    const yMin=Math.max(0,Math.floor((minSales-500000)/500000)*500000);

    if(monthlyRef.current){
      chartsRef.current.monthly=new Chart(monthlyRef.current,{
        type:'bar',
        data:{labels,datasets:[
          {label:currentYear+' TY',data:allMonths.map(m=>conv(curMonthly[m]?.s||0)),backgroundColor:'rgba(232,78,27,0.25)',borderColor:'#E84E1B',borderWidth:1,borderRadius:4,order:2},
          {label:priorYear+' LY',data:allMonths.map(m=>conv(lyMonthly[m]?.s||0)),type:'line',borderColor:'#888',borderDash:[5,5],pointBackgroundColor:'#888',pointRadius:4,tension:0.3,fill:false,order:1},
          {label:budgetLabel+' Budget',data:allMonths.map(m=>conv(budMonthly[m]?.s||0)),type:'line',borderColor:'#d97706',borderDash:[3,3],pointBackgroundColor:'#d97706',pointRadius:4,tension:0.3,fill:false,order:0},
        ]},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{usePointStyle:true,pointStyle:'circle',padding:16,font:{size:11}}},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${fmt(c.raw)} ${curr}`}}},scales:{y:{min:yMin,ticks:{callback:v=>fmtM(v)},grid:{color:'#f0ebe5'}},x:{grid:{display:false}}}}
      });
    }
    ...
    if(mgrRef.current&&bumSorted.length){
      const mgrData=mgrMetric==='sales'?bumSorted.map(b=>totalForPct?b[1].s/totalForPct*100:0):mgrMetric==='margin'?bumSorted.map(b=>conv(b[1].g)):bumSorted.map(b=>b[1].s?b[1].g/b[1].s*100:0);
      chartsRef.current.mgr=new Chart(mgrRef.current,{
        ...
        options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>mgrMetric==='gm'||mgrMetric==='sales'?fmtP(c.raw):fmt(c.raw)+' '+curr}}},scales:{x:{ticks:{callback:v=>mgrMetric==='gm'||mgrMetric==='sales'?v+'%':fmtM(v)},grid:{color:'#f0ebe5'}},y:{grid:{display:false}}}}
      });
    }
    ...
    if(deptRef.current&&deptSorted.length){
      const dData=deptMetric==='sales'?deptSorted.map(d=>conv(d[1].s)):deptMetric==='margin'?deptSorted.map(d=>conv(d[1].g)):deptSorted.map(d=>d[1].s?d[1].g/d[1].s*100:0);
      chartsRef.current.dept=new Chart(deptRef.current,{
        ...
        options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>deptMetric==='gm'?fmtP(c.raw):fmt(c.raw)+' '+curr}}},scales:{x:{ticks:{callback:v=>deptMetric==='gm'?v+'%':fmtM(v)},grid:{color:'#f0ebe5'}},y:{grid:{display:false}}}}
      });
    }
  },[filtered,priorFiltered,budgetFiltered,currentYear,priorYear,budgetLabel,mgrMetric,deptMetric,selectedMonths,salesType,marginType,isYTD,ytdProration,store,conv,curr]);

  useEffect(()=>{if(data.length)renderCharts()},[renderCharts,data.length]);

  // Detail table: aggregate by department (not by month)
  const tableData=useMemo(()=>{
    const agg={};
    filtered.forEach(r=>{
      const key=`${r.dept_name}-${r.bum}`;
      if(!agg[key])agg[key]={dept:r.dept_name,bum:r.bum,net_sales:0,gross_margin:0,dept_code:r.dept_code};
    ...
    priorFiltered.forEach(r=>{
      const key=`${r.dept_name}-${r.bum}`;
      if(!lyAgg[key])lyAgg[key]={net_sales:0,gross_margin:0};
      lyAgg[key].net_sales+=parseFloat(r.net_sales);
      lyAgg[key].gross_margin+=parseFloat(r.gross_margin);
    });
    // Budget per dept
    const budAgg={};
    budgetFiltered.forEach(b=>{
      const key=`${b.dept_code}`;
      if(!budAgg[key])budAgg[key]={sales:0,margin:0};
      if(b.budget_type===salesType)budAgg[key].sales+=parseFloat(b.amount);
      if(b.budget_type===marginType)budAgg[key].margin+=parseFloat(b.amount);
    });

    return Object.values(agg).map(row=>{
      const lyKey=`${row.dept}-${row.bum}`;
      let ly=lyAgg[lyKey]?.net_sales||0;
      let lyGm=lyAgg[lyKey]?.gross_margin||0;
      // Prorate LY for YTD
      if(isYTD&&ytdProration.month){
        const lyLastMonth=priorFiltered.filter(r=>r.month===ytdProration.month&&r.dept_name===row.dept&&r.bum===row.bum);
        const lyLastSales=lyLastMonth.reduce((s,r)=>s+parseFloat(r.net_sales||0),0);
        const lyLastGm=lyLastMonth.reduce((s,r)=>s+parseFloat(r.gross_margin||0),0);
        ly-=lyLastSales*(1-ytdProration.dayFraction);
        lyGm-=lyLastGm*(1-ytdProration.dayFraction);
      }
      const varPct=ly?((row.net_sales-ly)/Math.abs(ly)*100):0;
      const gmPct=row.net_sales?row.gross_margin/row.net_sales*100:0;
      const bud=budAgg[row.dept_code]||{sales:0,margin:0};
      let budGmPct=bud.sales?bud.margin/bud.sales*100:0;
      return{...row,ly:conv(ly),varPct,gmPct,net_sales_conv:conv(row.net_sales),gm_conv:conv(row.gross_margin),budMargin:conv(bud.margin),budGmPct};
  ...
  },[filtered,priorFiltered,budgetFiltered,search,sortCol,sortDir,isYTD,ytdProration,conv,salesType,marginType]);
  ...
  return(
    <div className="max-w-[1520px] mx-auto" style={{fontFamily:"'DM Sans',-apple-system,sans-serif",color:'#1a0a04'}}>
      ...
      <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-4 mb-5 space-y-3 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-1 flex-wrap">
            <Pill label="Alle" active={months.includes('all')} onClick={()=>setMonths(['all'])}/>
            <Pill label="YTD" active={months.includes('ytd')} onClick={()=>setMonths(['ytd'])}/>
            {MN.map((m,i)=><Pill key={i} label={m} active={months.includes(String(i+1))} onClick={(e)=>handleMonthClick(String(i+1),e)}/>)}
          </div>
          <span className="text-[10px] text-[#a08a74] ml-2">Ctrl+klik voor meerdere</span>
        </div>
        <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <KPI label="Netto Omzet" value={fmtMC(totalSales)} ly={fmtMC(lySales)} varLy={pctChange(totalSales,lySales)} budget={fmtMC(budgetSales)} budgetLabel={budgetLabel} varBudget={pctChange(totalSales,budgetSales)}/>
        <KPI label="Bruto Marge" value={fmtMC(totalGM)} ly={fmtMC(lyGM)} varLy={pctChange(totalGM,lyGM)} budget={fmtMC(budgetMargin)} budgetLabel={budgetLabel} varBudget={pctChange(totalGM,budgetMargin)}/>
      ...
      <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-5 shadow-sm mb-5">
        ...
        <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {[['Departement','dept'],['Manager','bum']].map(([l,k])=>(
                ...
                {[['Omzet','net_sales_conv'],['LY','ly'],['Var %','varPct'],['BM','gm_conv'],['Bud BM','budMargin'],['BM %','gmPct'],['Bud BM %','budGmPct']].map(([l,k])=>(
                  <th key={k} onClick={()=>toggleSort(k)} className="text-right p-3 text-[11px] text-[#6b5240] font-bold uppercase tracking-[0.6px] border-b-2 border-[#e5ddd4] bg-white sticky top-0 cursor-pointer hover:text-[#E84E1B] whitespace-nowrap">{l}{sortCol===k?(sortDir==='desc'?' ↓':' ↑'):''}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tableData.slice(0,tableRows).map((r,i)=>{
                ...
                const budGmColor=r.budGmPct>=35?'#16a34a':r.budGmPct>=25?'#d97706':'#dc2626';
                return(
                  <tr key={i} className="hover:bg-[#faf5f0]">
                    ...
                    <td className="p-2.5 border-b border-[#e5ddd4] text-right">
                      <span className="font-mono text-[13px]" style={{color:budGmColor}}>{fmtP(r.budGmPct)}</span>
                    </td>
                  </tr>
                );
              })}
