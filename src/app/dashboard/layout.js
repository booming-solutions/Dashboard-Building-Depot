...
function LoginModal({ show, onClose, supabase, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
      <div className="bg-white rounded-2xl p-7 w-[360px] shadow-xl" onClick={e => e.stopPropagation()}>
        <h3 className="text-[16px] font-bold text-[#1a0a04] mb-1">Inloggen</h3>
        <p className="text-[13px] text-[#6b5240] mb-4">Log in met je account</p>
        <input
          type="email" value={email} onChange={e => { setEmail(e.target.value); setError(''); }}
          className="w-full px-4 py-2.5 rounded-lg border border-[#e5ddd4] text-[14px] mb-3 focus:outline-none focus:border-[#1B3A5C]"
          placeholder="E-mailadres"
        />
        <input
          type="password" value={password} onChange={e => { setPassword(e.target.value); setError(''); }}
          onKeyDown={e => e.key === 'Enter' && handleLogin()}
          className="w-full px-4 py-2.5 rounded-lg border border-[#e5ddd4] text-[14px] mb-2 focus:outline-none focus:border-[#1B3A5C]"
          placeholder="Wachtwoord"
        />
        {error && <p className="text-[12px] text-red-500 mb-2">{error}</p>}
        <div className="flex gap-2 mt-3">
          <button onClick={() => { onClose(); setEmail(''); setPassword(''); setError(''); }}
            className="flex-1 py-2.5 rounded-lg bg-[#faf7f4] text-[#6b5240] text-[13px] font-semibold border border-[#e5ddd4]">
            Annuleren
          </button>
          <button onClick={handleLogin} disabled={loading}
            className="flex-1 py-2.5 rounded-lg bg-[#1B3A5C] text-white text-[13px] font-semibold disabled:opacity-50">
...
export default function DashboardLayout({ children }) {
  ...
  // Exacte match voor Overzicht (alleen /dashboard, niet /dashboard/*)
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col transition-all duration-300 fixed h-full z-40`} style={{ background: 'linear-gradient(180deg, #e8eff7 0%, #dce6f1 100%)' }}>
        ...
        <nav className="flex-1 py-4 px-3 overflow-y-auto">
          <div className="space-y-1">
            ...
            {/* Overzicht — exacte match alleen op /dashboard */}
            <Link href="/dashboard"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                isOverzicht ? 'bg-[#1B3A5C] text-white' : 'text-[#1B3A5C]/60 hover:text-[#1B3A5C] hover:bg-white/50'
              }`}>
              <span className="text-base flex-shrink-0">📊</span>
              {sidebarOpen && <span>Overzicht</span>}
            </Link>
        ...
        {/* Gebruiker info + login/uitloggen */}
        <div className="p-4 border-t border-[#c5d4e6]">
          {user ? (
            ...
            <button onClick={() => setShowLogin(true)} className="flex items-center gap-2 text-sm text-[#1B3A5C]/70 hover:text-[#1B3A5C] transition-colors w-full px-1 py-1.5 rounded-lg hover:bg-white/50">
              <span>🔑</span>
              {sidebarOpen && <span className="font-medium">Inloggen</span>}
            </button>
          )}
        </div>
      </aside>
