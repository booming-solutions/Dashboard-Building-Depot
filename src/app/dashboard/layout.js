...
/* ─── Dropdown nav item met sub-items ─── */
    ...
    return (
      <div className="relative group">
        ...
        <div className="absolute left-full top-0 ml-2 bg-white rounded-lg shadow-xl border border-gray-200 py-1 min-w-[200px] hidden group-hover:block z-50">
          ...
          {items.map(item => (
            <Link key={item.href} href={item.href} className={`block px-3 py-2 text-sm transition-all ${pathname === item.href ? 'bg-[#1B3A5C]/10 text-[#1B3A5C] font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-[#1B3A5C]'}`}>
              {item.label}
            </Link>
          ))}
        </div>
  ...
  return (
    <div>
      <button onClick={() => setOpen(!open)}
        ...
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" className={`transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col transition-all duration-300 fixed h-full z-40`} style={{ background: 'linear-gradient(180deg, #e8eff7 0%, #dce6f1 100%)' }}>
        ...
        <nav className="flex-1 py-4 px-3 overflow-y-auto">
          <div className="space-y-1">
            ...
            {/* Overzicht — EXACTE match op /dashboard alleen */}
            <Link href="/dashboard"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                pathname === '/dashboard' ? 'bg-[#1B3A5C] text-white' : 'text-[#1B3A5C]/60 hover:text-[#1B3A5C] hover:bg-white/50'
              }`}>
            ...
            <Link href="/dashboard/reports"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                pathname === '/dashboard/reports' ? 'bg-[#1B3A5C] text-white' : 'text-[#1B3A5C]/60 hover:text-[#1B3A5C] hover:bg-white/50'
              }`}>
              <span className="text-base flex-shrink-0">📋</span>
              {sidebarOpen && <span>Rapportages</span>}
            </Link>

            {/* Bestanden */}
            <Link href="/dashboard/files"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                pathname === '/dashboard/files' ? 'bg-[#1B3A5C] text-white' : 'text-[#1B3A5C]/60 hover:text-[#1B3A5C] hover:bg-white/50'
              }`}>
              <span className="text-base flex-shrink-0">📁</span>
              {sidebarOpen && <span>Bestanden</span>}
            </Link>
          </div>
        ...
        {/* ─── ONDERKANT SIDEBAR: login/logout ─── */}
      ...
      {/* Voettekst */}
      <div style={{
        position: 'fixed', bottom: 0, left: sidebarOpen ? '256px' : '80px', right: 0,
        background: 'linear-gradient(135deg, #152238 0%, #1B2E4A 100%)', borderTop: '1px solid rgba(75,163,212,0.15)',
        padding: '10px 32px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', zIndex: 100, transition: 'left 0.3s',
      }}>
