...
// vlakke tijdlijn van beschikbare maanden (chronologisch)
function buildTimeline(META){const tl=[];for(const y of META.years){const n=META.periodsAvailable[y]||12;for(let m=1;m<=n;m++)tl.push({y,m,label:MONTHS[m-1]+'/'+String(y).slice(2)});}return tl;}
// standaard 'tot' = laatst bekende afgesloten maand (april 2026 indien beschikbaar)
function balDefaultTo(META){const tl=buildTimeline(META);const want=tl.findIndex(t=>t.y==='2026'&&t.m===Math.min(4,(META.periodsAvailable['2026']||12)));return want>=0?want:tl.length-1;}
...
export default function StatementsDashboard(){
  ...
  const [balFromIdx,setBalFromIdx]=useState(()=>balDefaultTo(META)-2<0?0:balDefaultTo(META)-2);
  const [balToIdx,setBalToIdx]=useState(()=>balDefaultTo(META));
  ...
  // maand-tijdlijn + gekozen bereik (nieuwste eerst)
  const timeline=buildTimeline(META);
  const lastIdx=timeline.length-1;
  let toI=Math.min(Math.max(balFromIdx,balToIdx),lastIdx);
  let fromI=Math.max(0,Math.min(balFromIdx,balToIdx));
  fromI=Math.min(fromI,toI);
  const cols=[]; for(let i=toI;i>=fromI;i--)cols.push(timeline[i]);   // nieuwste → oudste
  const nCol=cols.length;

  const balRows=[];
  balDefs.forEach((d,bi)=>{
    if(d.g){balRows.push(<tr className="sd-group" key={'bg'+bi}><td colSpan={nCol+1}>{d.g}</td></tr>);return;}
    ...
    balRows.push(<tr className={cls.join(' ')} key={'br'+bi}>
      <td className="lbl stick">{d.label}</td>
      {cols.map((c,ci)=><td className={'r'+(ci===0?' newest':'')} key={'bc'+bi+'_'+ci}><Num v={d.fn(c.y,c.m)} tot={d.o.tot}/></td>)}
    </tr>);
  });
  const setSpan=(n)=>{setBalFromIdx(Math.max(0,toI-(n-1)));};
  ...
  <div className="sd-root">
    ...
    <div className="sd-panel">
      ...
      {tab==='balans'&&<>
        <div className="bal-ctrls">
          <label>Van maand
            <select value={fromI} onChange={e=>setBalFromIdx(+e.target.value)}>
              {timeline.map((t,i)=><option key={i} value={i} disabled={i>toI}>{t.label}</option>)}
            </select></label>
          <label>Tot maand
            <select value={toI} onChange={e=>setBalToIdx(+e.target.value)}>
              {timeline.map((t,i)=><option key={i} value={i} disabled={i<fromI}>{t.label}</option>)}
            </select></label>
          <div className="cgrp"><span className="clbl">Snel</span>
            <div className="sd-seg">
              {[3,6,12].map(n=><button key={n} className={nCol===n?'on':''} onClick={()=>setSpan(n)}>{n} mnd</button>)}
            </div></div>
          <span className="bal-hint">{nCol} maanden · nieuwste eerst · × 1.000 {curLabel}</span>
        </div>
        <div className="tablewrap"><table className="sd-table balmx">
          <thead><tr><th className="stick">Post</th>{cols.map((c,ci)=><th className={'r'+(ci===0?' newest':'')} key={'h'+ci}>{c.label}</th>)}</tr></thead>
...
.bal-ctrls{display:flex;flex-wrap:wrap;gap:12px 16px;align-items:flex-end;padding:10px 8px 14px}
.bal-ctrls label{display:flex;flex-direction:column;font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;color:#6a7785;gap:4px;font-weight:600}
.bal-ctrls select{font-family:inherit;font-size:13px;color:var(--ink);background:var(--card);border:1px solid var(--line);border-radius:7px;padding:6px 9px;min-width:110px}
.bal-hint{font-size:11px;color:#9aa3ad;margin-left:auto;align-self:center}
.sd-table.balmx th,.sd-table.balmx td{white-space:nowrap}
.sd-table.balmx th.r,.sd-table.balmx td.r{min-width:78px}
.sd-table.balmx .stick{position:sticky;left:0;background:var(--card);z-index:2;text-align:left}
.sd-table.balmx thead .stick{z-index:3}
.sd-table.balmx tr.sd-group .stick,.sd-table.balmx tr.sd-group td{background:#f3eee2}
.sd-table.balmx tr.tot .stick{background:#f3eee2}
.sd-table.balmx tr.sub .stick{background:#faf7f0}
.sd-table.balmx th.newest,.sd-table.balmx td.newest{background:#fbf6ec}
.sd-table.balmx th.newest{color:var(--gold2)}
