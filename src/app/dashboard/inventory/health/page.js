/* ============================================================
   BESTAND: page_inventory_health.js
...
export default function InventoryHealthDashboard() {
  ...
  async function loadData() {
    ...
    while (true) {
      var r = await supabase.from('buying_data').select('*').range(from, from + step - 1);
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><p className="text-[#6b5240]">Voorraad gezondheid laden...</p></div>;
  ...
  return (
    <div className="max-w-[1600px] mx-auto" style={{ fontFamily: "'DM Sans', -apple-system, sans-serif", color: '#1a0a04' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '22px', fontWeight: 900 }}>Gezondheid Voorraden</h1>
          <p className="text-[13px] text-[#6b5240]">{'Building Depot — ' + storeName + ' — analyse op basis van 12 maanden verkoophistorie'}</p>
        </div>
