...
/* ── Visual % bar with hover tooltip via state ── */
    ...
    <div className="flex items-center gap-2 py-[3px] relative" style={{ minHeight: '28px' }}
      ...
      {hovered && (
        <div style={{
          ...
          backgroundColor: '#1B3A5C', color: 'white', fontSize: '10px',
          fontFamily: 'monospace', padding: '5px 10px', borderRadius: '8px',
...
export default function InventoryDashboard() {
  ...
  /* ── Build trend data: total, per BUM, or per dept ── */
  var trendChartData = useMemo(function() {
    ...
    var labels = sortedDates.map(function(dt) {
      var p = dt.split('-');
    ...
    if (trendDept === '__total__') {
      // Total of all filtered departments
      ...
      values = sortedDates.map(function(dt) {
        var sum = 0;
        trendDepartments.forEach(function(d) {
      ...
      values = sortedDates.map(function(dt) {
        var h = dept.history.find(function(x) { return x.date === dt; });
        return h ? h.value : 0;
      });
  ...
  useEffect(function() {
    ...
    if (view !== 'trend' || !trendChartData) return;
  ...
  // For trend summary info
  var trendTotalBudget = 0, trendTotalActual = 0;
  trendDepartments.forEach(function(d) { trendTotalBudget += d.budget; trendTotalActual += d.actual; });
  var trendPct = trendTotalBudget ? ((trendTotalActual - trendTotalBudget) / trendTotalBudget) * 100 : 0;
    ...
    <div className="max-w-[1600px] mx-auto" style={{ fontFamily: "'DM Sans', -apple-system, sans-serif", color: '#1a0a04' }}>
      ...
      <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-4 mb-5 space-y-3 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-1">
            {stores.map(function(s) { return <Pill key={s} label={s === '1' ? 'Curaçao' : s === 'B' ? 'Bonaire' : s} active={store === s} onClick={function() { setStore(s); setSelBum('all'); }} />; })}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
        {[
          ...
          { label: 'Verschil', value: isBonaire ? 'n.v.t.' : ((totals.diff >= 0 ? '+' : '') + fmtK(totals.diff)), color: totals.diff > 0 ? '#dc2626' : '#16a34a' },
          { label: '% vs Budget', value: isBonaire ? 'n.v.t.' : fmtP(totals.pct), color: totals.pct > 0 ? '#dc2626' : '#16a34a' },
        ].map(function(k, i) {
          return (
              ...
              <tbody>
                <tr className="bg-[#faf7f4]">
                  ...
                  {historyDates.map(function(dt) {
                    var sum = 0; departments.forEach(function(d) { var h = d.history.find(function(x) { return x.date === dt; }); if (h) sum += h.value; });
      ...
      {/* ═══ VISUAL % BAR CHART ═══ */}
      {view === 'visual' && (
        <div className="bg-white rounded-[14px] border border-[#e5ddd4] shadow-sm p-6 mb-8" style={{ overflow: 'visible' }}>
          <h3 className="text-[15px] font-bold mb-1">Voorraad vs Budget per Departement</h3>
          <p className="text-[12px] text-[#6b5240] mb-2">Procentueel verschil t.o.v. budget — hover voor details</p>
          <div className="flex items-center gap-4 text-[10px] mb-4">
            ...
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm" style={{ backgroundColor: '#dc2626' }}></span> Buiten ±15%</span>
          </div>
          {!isBonaire ? (
        ...
        <div className="space-y-5 mb-8">
          <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-4 shadow-sm space-y-3">
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-[11px] text-[#6b5240] font-bold uppercase tracking-[0.8px] w-20">Store</span>
              <div className="flex gap-1">
                {stores.map(function(s) { return <Pill key={s} label={s === '1' ? 'Curaçao' : s === 'B' ? 'Bonaire' : s} active={trendStore === s} onClick={function() { setTrendStore(s); setTrendBum('all'); setTrendDept('__total__'); }} />; })}
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-[11px] text-[#6b5240] font-bold uppercase tracking-[0.8px] w-20">Manager</span>
            ...
            <div className="bg-white rounded-[14px] border border-[#e5ddd4] p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                {trendChartData.budgetValues[0] > 0 && (function() {
                  ...
                  return <span className={'text-[20px] font-bold font-mono ' + (p > 0 ? 'text-red-600' : 'text-green-600')}>{(p >= 0 ? '+' : '') + fmtP(p)}</span>;
                })()}
              </div>
